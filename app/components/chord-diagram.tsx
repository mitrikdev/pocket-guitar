...
export function ChordDiagram({ voicing, label, compact = false }: { voicing: ChordVoicing; label: string; compact?: boolean }) {
  ...
  const rows = Math.max(5, highest - baseFret + 1)
  const bottom = 35 + rows * 22
  ...
  return <svg className={'chord-diagram' + (compact ? ' compact' : '')} viewBox={'0 0 162 ' + (bottom + 21)} role="img" aria-label={label + ', ' + voicing.name + '. Low E to high E: ' + spoken}>
    <title>{label + ' — ' + voicing.name}</title>
    {baseFret > 1 ? <text x="4" y="50" className="diagram-fret-label">{baseFret}fr</text> : null}
    {Array.from({ length: rows + 1 }, (_, i) => i).map(line => <path key={'fret' + line} className={line === 0 && baseFret === 1 ? 'diagram-nut' : 'diagram-wire'} d={'M28 ' + (35 + line * 22) + 'H133'}/>)}
    {[0, 1, 2, 3, 4, 5].map(string => <path key={'string' + string} className="diagram-wire" d={'M' + x(string) + ' 35V' + bottom}/>)}
    ...
    {['E', 'A', 'D', 'G', 'B', 'e'].map((name, index) => <text key={index} x={x(index)} y={bottom + 16} className="diagram-string-label">{name}</text>)}
  </svg>
}
